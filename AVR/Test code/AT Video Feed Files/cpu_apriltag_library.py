...
class AprilTagVPS:
    def __init__(
        ...
        # Create MQTT object for broadcasting camera feed
        self.broadcaster = MQTTModule()
    ...
    def capture_loop(self) -> None:
        ...
        while True:
            ...
            color_ret, color_img = capture.read()

            if color_ret: # If we have a valid image, encode and broadcast it (This will need to be decoded at the destination)
                _, buffer = cv2.imencode(".jpg", color_img)
                encoded_img = base64.b64encode(buffer)
                b64_str = encoded_img.decode("utf-8") # Encode as a string, I added this part because that's what the thermal camera does (See thermal/thermal.py)
                self.broadcaster.send_message('avr/apriltags/camfeed', {'img': b64_str})
                """
                I used this article (https://medium.com/@pritam.mondal.0711/stream-live-video-from-client-to-server-using-opencv-and-paho-mqtt-674d3327e8b3)
                to write this part, check it out and compare it to the thermal camera encoding/decoding process to write the decoding for this
                """
